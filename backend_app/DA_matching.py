...
def matching(students, sekis):
    poolList = students
    differdAccept = sekis

    # poolListが空になる（全員の配属が決定）まで提案と諾否の決定を続ける。
    while len(poolList)>0:
        seki = next(seki for seki in differdAccept if seki.name == poolList[0].tempPref())
        index = sekis.index(seki)
        select = seki.choice(poolList[0])
        if select[0] == None:
            del poolList[0]
            differdAccept[index] = seki
        else:
            del poolList[0]
